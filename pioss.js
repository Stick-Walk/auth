...
(function () {
	var callbacks = {};
	var idCounter = 0;
	var flashUrl = "https://192.168.30.154/html5client/FlashFallback/bin-debug/FlashFallback.swf"
	...
	_pio.gameFS = function(gameId, redirectMap){
		...
		this.getUrl = function (path, secure) {
			...
			if( map ){
				...
				return (secure ? "https" : "https") + "://r.playerio.com/r/" + gameId + path;
			}
		}
	}
})();
...
(function () {
	...
	*/
	...
	_pio.showDialog = function(dialog, channel, args, closedCallback){
		if (typeof (window.PublishingNetwork) == 'undefined') {
			throw new PlayerIOError(PlayerIOErrorCode.PublishingNetworkNotAvailable, "PublishingNetwork.js was not found on the current page. You have to include the 'piocdn.com/publishingnetwork.js' on the containing page to show dialogs. See https://playerio.com/documentation/ for more information.")
        } else {
...
(function () {
	...
	*	name: 'Buy 200 coins', 
	*	description: '...', 
	*	icon: 'https://server.com/icon.jpg', 
	*	currency:'USD'
	* },function(){
	*	// payment success
	* }, function (err) { console.log(err) })
